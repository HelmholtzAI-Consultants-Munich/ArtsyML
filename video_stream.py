...
from model.net import style_model
...
def get_args():
    ...
    parser.add_argument('--style_img', default='./images_style/picasso.jpg')
...
if __name__ == '__main__':
    ...
    ts_transform = transforms.Compose(ts_transform_list)
    style_image = Image.open(str(style_path)).convert('RGB')
    style_image = ts_transform(style_image)
    ...
    # --- PyTorch version of Style-Transfer
    decoder = net.decoder
    vgg = net.vgg
    vgg.load_state_dict(torch.load('./model/saved_models/vgg_normalized.pth'))
    decoder.load_state_dict(torch.load('./model/saved_models/decoder_iter_10000.pth.tar'))
    vgg = nn.Sequential(*list(vgg.children())[:31])
    # style_model = net.Net(vgg, decoder)
    # ---
    ...
    while (True):
        ...
        content_image = ts_transform(torchvision.transforms.ToPILImage()(frame))
        # content_image = content_image.unsqueeze(0)
        # style_image = style_image.unsqueeze(0)

        # style_image_tensor = style_model(tf.constant(content_image), tf.constant(style_image))[0]
        style_image_tensor = style_model(vgg, decoder, content_image, style_image, alpha=1.0,
        ...
        frame = cv2.resize(frame, (style_img.shape[2], style_img.shape[1]))
        ...
        style_img = np.transpose(style_img, (1, 2, 0))
